                        ...
                        <div class="collapse mt-4" id="formNuevaDireccion">
                            <div class="card card-body border-0 shadow-sm rounded-4" style="background-color: #f8fafc;">
                                ...
                                <form action="{{ route('profile.address.store') }}" method="POST">
                                    @csrf
                                    <x-input-label for="receptor_name" value="Nombre de quien recibe" />
                                    <x-text-input id="receptor_name" name="receptor_name" type="text" class="mt-1 block w-full" :value="old('receptor_name')" required />
                                    <x-input-error :messages="$errors->get('receptor_name')" class="mt-2" />

                                    <div class="mt-4">
                                        <x-primary-button>Guardar Dirección</x-primary-button>
                                    </div>
                                </form>
                            </div>
                        </div>
